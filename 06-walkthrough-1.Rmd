...
```{r}
# may want to use something other than car; sjmisc has a useful function for this
s12_pre_survey $Q1MaincellgroupRow4_rc <- car::recode(s12_pre_survey $Q1MaincellgroupRow4, "1=5; 2=4; 5=1; 4=2")
s12_pre_survey $Q1MaincellgroupRow7_rc <- car::recode(s12_pre_survey $Q1MaincellgroupRow7, "1=5; 2=4; 5=1; 4=2")
...
s12_pre_survey $int <- (s12_pre_survey $Q1MaincellgroupRow1 + s12_pre_survey $Q1MaincellgroupRow4_rc + s12_pre_survey $Q1MaincellgroupRow8 + s12_pre_survey $Q1MaincellgroupRow10+ s12_pre_survey $Q1MaincellgroupRow5) / 5
s12_pre_survey $uv <- (s12_pre_survey $Q1MaincellgroupRow2 + s12_pre_survey $Q1MaincellgroupRow6+ s12_pre_survey $Q1MaincellgroupRow9) / 3
s12_pre_survey $percomp <- (s12_pre_survey $Q1MaincellgroupRow3 + s12_pre_survey $Q1MaincellgroupRow7_rc) / 2
s12_pre_survey $tv <- (s12_pre_survey $Q1MaincellgroupRow1 + s12_pre_survey $Q1MaincellgroupRow8 + s12_pre_survey $Q1MaincellgroupRow10+ s12_pre_survey $Q1MaincellgroupRow5 + s12_pre_survey $Q1MaincellgroupRow2 + s12_pre_survey $Q1MaincellgroupRow6+ s12_pre_survey $Q1MaincellgroupRow9) / 7
```
...
x <- str_split(s12_course_data$CourseSectionOrigID, "-")

s12_course_data <- mutate(s12_course_data,
                 subject = map_chr(x, ~ .[1]),
...
```{r}
s12_time_spent <- rename(s12_time_spent, student_id = Bb_UserPK, course_id = CourseSectionOrigID)
s12_time_spent <- mutate(s12_time_spent, student_id = as.integer(student_id))
dat <- dat %>% left_join(s12_time_spent, by = c("student_id", "course_id"))
```
...
Is this course-specific differences in how much time students' spend on the course--and in how time spent is related to the percentage of points studented earned.
