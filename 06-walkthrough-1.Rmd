...
s12_measure_mean <- s12_pre_survey %>% 
    ...
    mutate(
        # Initialize measure variable as character
        measure = "", 
        # Interest measure
        measure = ifelse(question %in% c("q1", "q4", "q5", "q8", "q10"), "int", measure), 
        # Utility value measure
        measure = ifelse(question %in% c("q2", "q6", "q9"), "uv", measure), 
        # Perceived competence measure
        measure = ifelse(question %in% c("q3", "q7"), "pc", measure)
        ) %>% 
    group_by(measure) %>% 
    summarise(
