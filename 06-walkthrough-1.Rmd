...
Specifically, we can dummy-code the groups, which means that each group will be associated with a different intercept. We will save this to an object that we will call `m_linear_dc`, where the `dc` stands for dummy code.

<!-- Will probably need to walk through this a lot more slowly - and so not sure that this chapter belongs here -->

```{r}
m_linear_dc <- lm(percentage_earned ~ TimeSpent + course_id, data = dat)
```

The output will be a bit, well, long, because each group will have its own intercept. Here it is:

<!-- Wonder if there's a package we can use to print these table in a prettier way -->
