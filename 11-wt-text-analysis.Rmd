...
For this analysis, we'll be using the {tidyverse}, {here}, and {dataedu} packages. We will also use the {tidytext} package for working with textual data [@R-tidytext]. As it has not been used previously in the book, you may nneed to install the {tidytext} package (and - if you haven't just yet - the other packages), first. 
...
Let's start by getting the data into our programming environment so we can start analyzing it. In [Chapter 12](#c12), we describe how to access this data through Twitter Application Programming Interface, or API. We've included the raw dataset of TidyTuesday tweets in the {dataedu} package. You can see the dataset by typing `tt_tweets`. Let's start by assigning the name `raw_tweets` to this dataset:
...
```{r fig11-1, results='hide', message = FALSE, fig.cap="Count of Words Associated with Positivity"}
pos_tokens_count %>%
