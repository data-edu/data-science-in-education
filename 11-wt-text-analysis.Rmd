...
In this chapter, we focus on analyzing text data from Twitter. We focus on this particular data *source* because we think it is relevant to a number of educational topics and questions, including how newcomers learn to visualize data. Twitter data is complex and includes not only information about who posted a tweet and when, it also includes the text of the tweet (and a great deal of additional information, see [@R-rtweet]). This makes it especially well-suited for exploring the uses of text analysis. 
