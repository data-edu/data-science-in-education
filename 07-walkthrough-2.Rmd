...
You can use the ggplot function to graph continuous variables onto a scatterplot. The x-axis shows the learners' overall grade in percentages while the y-axis lists each individual learner. The argument `color = running_average > 90` colors the points to helps highlight out which learners have overall grades above 90%.

```{r echo = TRUE, message = FALSE, warning = FALSE}
# Scatterplot of continuous variable
gradebook %>% 
  ggplot() + 
  geom_point(aes(x = running_average, 
                 y = name, 
                 color = running_average > 90)) +
  labs(title = "Scatterplot of Student Grades", 
       x = "Overall Grade in Percentage", 
       y = "Name", 
       color = "90% or Above") +
  theme_dataedu() +
  scale_color_dataedu()
...
gradebook %>% 
  ggplot() + 
  geom_point(aes(x = homeworks, 
                 y = running_average)) +
  geom_smooth(aes(x = homeworks, 
                  y = running_average)) +
  labs(title = "Correlation Between Overall Grade and Homework", 
       x = "Homework Grade in Percentage", 
