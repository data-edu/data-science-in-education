...
For the purposes of this walkthrough, you'll be looking at a school district's data. In the next, you'll zoom out and look across states in the United States.
...
The district has since changed its reporting process to a school-level dashboard. The PDFs we used in the first edition are no longer available on the district's website. This is a common occurrence in education. We cannot assume that data will remain available in the same format or location, or remain available at all.

Adapting to changes in data availability is crucial. This includes implementing data governance measures in your organization, saving backups in an organized manner in accordance with privacy and security policies, and creating a centralized repository for data-related resources. To learn more, we recommend Data Management in Large-Scale Education Research by Crystal Lewis [@lewis2024data].
...
In this chapter, you'll practice analyzing data from a single district to help education data practitioners describe the district's needs and opportunities. You'll use descriptive analysis on an aggregate dataset to find out whether there is a phenomenon present, what it is, and what leaders can address through supports, reforms, or interventions [@descriptive]. This is different from causal analyses, which aims to assess the root cause of a phenomenon or the effects of an intervention. 

To begin, you'll learn how to import PDFs into R, clean and wrangle data, and visualize it to gain insights about the district.

## Load packages

As in past chapters, begin your script by calling the packages you'll need. If you have not installed these packages yet, see the ["Packages" section](#c06p) of the ["Foundational Skills"](#c06) chapter.
...
(https[]()://github.com/ropensci/tabulapdf) package [@sepulveda]. It provides R bindings to the Tabula PDF Table Extractor Library, which can be used to computationally extract tables from PDF documents. It is not available on CRAN, so install it by running the code below:
...
You have three options for extracting the data from the PDFs. Pick one option and run the code, then follow the data workflow to clean clean and process the data. 
...
```{r, eval = FALSE}
# Option 1: Get data using {tabulizer}
race_pdf <-
    extract_tables(
...
The object `race_pdf` is a list. Before analyzing the data, you need to transform the list to a data frame. Start by turning `race_pdf` into a tibble by using `map(as_tibble())` from the {purrr} package. Then, use the `list_rbind()` function to turn these tibbles into a single data frame. Next, use the `slice()` function to remove unnecessary rows. Finally, create readable column names using `set_names()`.
