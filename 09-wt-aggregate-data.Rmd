...
In this walkthrough, you'll encounter the term "disaggregated." In the context of the walkthrough, this means disaggregating to some smaller group, but not to the student level. 
...
For the Race/Ethnicity table, you want the totals for each district school because you won't be looking at grade-level variation. When looking at the resulting data frame, you can see the school totals have "Total" in `school_name`.
...
2.  Remove all "Grand Total" rows using `filter()`. Keep schools that have "Total" in the name but remove any rows that are "Grand Total".
3.  Trim white space from character columns using `trimws()`.
4.  Turn the data in the `percentage` columns into a numeric data type. These columns contain percentage signs, meaning they were read in as characters.
...
FRPL data are often used as a proxy for poverty [@frpl]. Students from a household with an income up to 185% of the poverty threshold are eligible for free or reduced price lunch. 
...
Similar to the Race/Ethnicity PDF, you'll take the PDF list output, turn it into tibbles, then create a single data frame. Then you'll remove unnecessary rows from each page using `slice()`. Finally, you'll create column names that are easily understood.
...
To clean the dataset further, remove the blank rows. Notice that there are aggregations inserted into the table that are not at the district level. For example, the report includes `ELM K_08` as a value of the variable `school_name`, presumably to aggregate FRPL numbers up to the K--8 level. Although this is useful data, you won't need it for this district-level analysis. Filter them out by inserting `!` before the variable name.
...
With both the Race/Ethnicity and FRPL data ready, you'll join the two to prepare them for analysis. Join the two datasets by the name of the school. To make the student counts and percentages numeric, apply `as.numeric` to multiple columns using `mutate_at()`.
...
Did you notice? The total number of students from the Race/Ethnicity table does not match the total number of students from the FRPL table, even though they're referring to the same districts in the same year. 

Perhaps the two datasets were created using different rules when aggregating the dataset. Perhaps the counts were taken at different times of the year and students moved around in the meantime. Even if you don't know the cause, you'll need to make strategic decisions about which data to consider the "truth" in this analysis.
...
Look at these data and note the shares of each race group. Almost 40% of students are Black, and around 36% are White. This matches the percentages provided in the original PDFs, which validates the calculations you did when you cleaned the data. Good job!
...
56.9% of the students are eligible for FRPL, compared to the US average of 52.1% [@avgfrpl]. This also validates what you saw in the original PDFs. 

### Analyzing spread

Now that you've done some initial validations, try exploring the dataset through distributions. This empowers you to see beyond district totals and into the underlying school data. 
...
Try the approach you used to look at the distribution of student race groups in all schools to look at the distribution of of student race groups in high-poverty schools. 
...
So far, you've validated your initial calculations and explored distributions. Next, you'll explore correlations. 
...
1. The distribution of race/ethnicity within schools is not representative of the district.
