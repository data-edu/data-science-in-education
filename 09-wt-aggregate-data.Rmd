...
make sure we're getting the data that we expect. We then transform the list to a data frame with `map(as_tibble)`.  The `slice()` in `map_df()` removes unnecessary rows from the PDF. Finally, we create readable column names using `set_names()`.
...
```{r fig9-1, fig.align = "center", results = "hide", message = FALSE, fig.cap = "Percentage of Population by Subgroup"}
district_tidy_df %>%
...
```{r fig9-2, fig.align = "center", results = 'hide', message = FALSE, fig.cap = "Count of Schools by White Population"}
district_merged_df %>%
...
```{r fig9-3, fig.align = "center", results = 'hide', message = FALSE, fig.cap = "Distribution of Subgroups in High Poverty Schools"}
district_tidy_df %>%
  filter(school_name == "Total",
...
```{r fig9-4, warning = FALSE, fig.align = "center", message = FALSE, results = 'hide', fig.cap="FRPL Percentage vs. White Percentage"}
district_merged_df %>%
  filter(school_name != "Total") %>%
