...
ROpenSci created the [{tabulapdf}](https://github.com/ropensci/tabulapdf)(https[]()://github.com/ropensci/tabulapdf) package [@sepulveda]. It provides R bindings to the Tabula PDF Table Extractor Library, which can be used to computationally extract tables from PDF documents. It is not available on CRAN, so install it by running the code below:
...
56.9% of the students are eligible for FRPL, compared to the US average of 52.1% [@avgfrpl]. These numbers also validates what you saw in the original PDFs. 
