...
Using a freely-available school district dataset, this chapter looks at the distribution of students in the district by race and socioeconomic status. Subgroup analysis can point out the state of equity in a system and inform improvement for students. Data science techniques in this chapter include reading tables from an online PDF into a machine-readable format, preparing data for analysis, transforming it into a tidy format, visualizing it, and analyzing distributions and relationships.
...
Aggregate data is essential both for accountability and for providing useful information about schools and districts. For example, district administrators might aggregate student-level enrollment reports over time. This allows them to see how many students enroll in each school, in the district overall, and at various grade levels. Depending on the state, the district administrator might submit these aggregate data to their state education agency (SEA) for reporting purposes. These datasets might be posted on the state's Department of Education website for anyone to download and use.

Federal and international education datasets provide additional information. In the US, some federal datasets aim to consolidate important metrics from all states. This can be useful because each state has its own repository of data. Further, going through each state's website to download a particular metric is a significant effort. However, federal datasets often have more stringent data requirements than the states, so the datasets may be less usable.

Data scientists in education can analyze these reports and datasets to
answer questions related to their field of interest. However, doing so is not always straightforward. Publicly available aggregate datasets are large and often suppressed to protect privacy. Sometimes, agencies release multiple years behind the current year. Because of their coarseness, they can be difficult to interpret and use.

Generally, aggregate data is used to surface broader trends and patterns in education instead of diagnosing underlying issues or making causal statements. It is very important that data scientists consider the limitations of aggregate data before analyzing it.
...
This chapter and the [following one](#c10) provide two different examples of cleaning aggregate data and using these datasets to compare student experiences. In this chapter, you'll focus on educational equity by identifying and comparing patterns in student demographic groups. In the next chapter, you'll compare student counts over time in different states.
...
On the federal level, well-known examples include:
  ...
  - [Civil Rights Data Collection (CRDC)](https://www2.ed.gov/about/offices/list/ocr/data.html)
    (https[]()://www2.ed.gov/about/offices/list/ocr/data.html), which reports many different variables on educational programs and services disaggregated by race/ethnicity, sex, limited English proficiency, and disability. These data are at the school level.
...
At the state and district levels, two examples include:
  ...
  - [California Department of Education](https://www.cde.ca.gov/ds/) (https[]()://www.cde.ca.gov/ds/), which is the state's Department of Education website. It includes both downloadable CSV files and "Data Quest", which lets you query the data online.

  - [Minneapolis Public Schools](https://www.mpschools.org/about-mps/data-dashboard) (https[]()://mpschools.org/about-mps/data-dashboard), which is a district-level website with datasets beyond those listed on the state website.
...
When we published the first edition of this book in 2020, the district provided comprehensive student demographic data in PDF format. They reported not only the percentage of students in each subgroup but also the exact number of students. This detailed information allowed a thorough examination of individual school demographics and offered an excellent opportunity to explore systemic inequities.

A few years ago, the district changed its reporting process to a school-level dashboard. The PDFs we used in the first edition are no longer available on the district's website. This is a common occurrence in education. We cannot always assume that data we have been using will remain available in the same format or location, or remain available at all.

Adapting to changes in data availability is crucial. To avoid future issues, we can implement data governance measures at our organization. These measures include regularly saving backups in an organized manner, provided the data is allowed to be stored in other locations, and creating a centralized repository for all data-related resources. To learn more, we recommend Data Management in Large-Scale Education Research by Crystal Lewis [@lewis2024data].

To ensure consistent access to the PDFs for this chapter, we saved copies in our  [Github repository](https://github.com/data-edu/data-science-in-education/tree/master/data/agg_data) (https[]()://github.com/data-edu/data-science-in-education/tree/master/data/agg_data). eYou can verify their original locations on the district website with the Wayback Machine:

* [Race demographic data](https://web.archive.org/web/20201112024712/https://studentaccounting.mpls.k12.mn.us/uploads/mps_fall2018_racial_ethnic_by_school_by_grade.pdf) (https[]()://web.archive.org/web/20201112024712/https[]()://studentaccounting.mpls.k12.mn.us/uploads/mps_fall2018_racial_ethnic_by_school_by_grade.pdf)
...
Below, we will demonstrate how to import PDFs into R, clean and wrangle the data, and then visualize and analyze it to gain insights about the district.

### Methods

In this chapter, we will walk through how running analyses on data from a single district can help education data practitioners *understand and describe the landscape of needs and opportunities* present there. As opposed to causal analyses, which aim to assess the root cause of a phenomenon or the effects of an intervention, we use descriptive analysis on an aggregate dataset to find out whether there *is* a phenomenon present, *what* it is, and *what* may be worth trying to address through future supports, reforms, or interventions [@descriptive].

## Load packages

As usual, we begin our script by calling the packages we will use. If you have not installed any of these packages yet, see the ["Packages" section](#c06p) of the ["Foundational Skills"](#c06) chapter.
...
ROpenSci created the [{tabulapdf}](https://github.com/ropensci/tabulapdf)
(https[]()://github.com/ropensci/tabulapdf) package [@sepulveda] which provides R bindings to the Tabula PDF Table Extractor Library, which can be used to computationally extract tables from PDF documents. It is not available on CRAN, so install it by running the code below:
...
{rJava}[@R-rJava] is a required package to load {tabulapdf}. Unfortunately, installing {rJava} can be very tedious. If you find yourself unable to install {rJava}, or would like to go straight to the data processing, you can skip the steps requiring {tabulapdf}. We provide the raw and processed data in the {dataedu} package.
...
You have three options for extracting the data from the PDFs:

1. You can use {tabulapdf}, which pulls the PDF tables into lists using `extract_tables()`. The PDFs are available in the book's [Github repository](https://github.com/data-edu/data-science-in-education/tree/master/data/agg_data) (https[]()://github.com/data-edu/data-science-in-education/tree/master/data/agg_data). Then, you can follow the data workflow below to clean and process the data.
2. You can get the imported but unprocessed data from the book's [Github repository](https://github.com/data-edu/data-science-in-education/tree/master/data/agg_data) (https[]()://github.com/data-edu/data-science-in-education/tree/master/data/agg_data). They are available in the files `race_pdf.Rds` and `frpl_pdf.Rds`. If you would like to set up the folders in your working directory in the same way they are in the book, first create a folder called `"data"`. Then, inside that folder, create a second folder called `"agg_data"` and place the `race_pdf.Rds` and `frpl_pdf.Rds` files in the `"agg_data"` folder. Then, you can load the data using `here()`. Otherwise, you will have to change the file path inside of `here()` to match where the data is stored on your working directory. 
3. Finally, you can get the imported but unprocessed data from the {dataedu} package.

First, let's look at the Race/Ethnicity tables. Import the PDF information using one of the three options:
...
The object `race_pdf` from above is a list. We can transform the list to a data frame making the matrix version of the PDFs into a tibble by using `map(as_tibble())` from the {purrr} package. Then, we use the `list_rbind()` function then turns these tibbles into a single data frame. The `slice()` function removes unnecessary rows from the tibbles. Finally, we create readable column names using `set_names()` (otherwise, they look like `...1`, `...2`, etc.).
...
For the Race/Ethnicity table, you want the totals for each district school, as you won't be looking at grade-level variation. When looking at the resulting data frame, you can see the school totals have "Total" in `school_name`.
...
2.  Removing all "Grand Total" rows (otherwise, they'll show up in your data when you just want district-level data) using `filter()`. Keep schools that have "Total" in the name but remove any rows that are "Grand Total".
3.  Trimming white space from character columns using `trimws()`.
4.  Turning the data in the `percentage` columns into numeric. Those columns contain percentage signs, meaning they were read in as characters. You will have to remove all of the non-numeric characters to be able to do math with these columns (for example, to add them together). Divide the numbers by 100 so they are in decimal format.

Let's break this line down: `mutate(across(matches("pct"), ~ as.numeric(str_replace(., "%", "")) / 100))`. We are telling `mutate` to:

* Use `match()` in conjunction with `across()` to select columns with names that match the regular expression "pct"
* For the rows in those columns, replace the character "%" with blanks "" by using `str_replace(., "%", "")`
* Make those rows numeric by using `as.numeric()`
* Divide those numbers by 100 using `/100`
...
Similar to the Race/Ethnicity PDF, you take the PDF list output, turn it into tibbles, then create a single data frame. Remove unnecessary rows from each page using `slice()`. Then, we create column names that are easily understood.
...
To clean the dataset up further, remove the blank rows. Notice that there are aggregations inserted into the table that are not at the district level. For example, the report includes `ELM K_08` as a value of the variable `school_name`, presumably to aggregate FRPL numbers up to the K--8 level. Although this is useful data, we don't need it for this district-level analysis. There are different ways we can remove these rows, but you can just filter them out by inserting `!` before the variable name.
...
To look at race and ethnicity data in conjunction with FPRL percentage, join the two datasets by the name of the school. To make the student counts and percentages numeric, apply `as.numeric` to multiple columns using `mutate_at()`.
...
1.  We want to calculate, for each race, the number of students in "high-poverty" schools. This is defined by NCES as schools that are over 75% FRPL [@ncesfrpl]. When a school is over 75% FRPL, we count the number of students for that particular race under the variable `[racename]_povnum`.
...
Running the code above, particularly the importing of the PDFs, takes a lot of time. We've saved copies of the merged and tidy data in the book's GitHub repository and {dataedu}. To access them, you can run the code below.
...
What do the racial demographics in this district look like? A barplot can quickly visualize the different proportions of subgroups.
...
When we look at these data, the district seems very diverse. Almost 40% of students are Black, and around 36% are White. Note that this matches the percentages provided in the original PDFs. This shows that your calculations above are accurate. Hooray!
...
**56.9% of the students are eligible for FRPL**, compared to the US average of 52.1% [@avgfrpl]. This also matches the numbers in the PDFs. Great!
...
**26 of the 74 (35%) of schools have between 0 and 10% White students.** This implies that even though the school district may be diverse, the demographics are not evenly distributed across the schools. More than half of schools enroll fewer than 30% of White students, even though White students make up 35% of the district student population.
...
Because of the disaggregated data this district provides, we can go deeper than the average demographics across the district and see what it looks like at the school level. These distinct but closely related views demonstrate that:

1. There exists a distribution of race/ethnicity within schools that is not representative of the district.
