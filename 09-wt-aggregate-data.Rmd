...
At the state and district levels, two examples include:
  ...
  - [Minneapolis Public Schools](https://www.mpschools.org/about-mps/data-dashboard)
    (https[]()://mpschools.org/about-mps/data-dashboard), which is a district-level
...
When we published the first edition of this book in 2019, the district provided comprehensive student demographic data in PDF format. They reported not only the percentage of students in each subgroup but also the exact number of students. This detailed information allowed a thorough examination of individual school demographics and offered an excellent opportunity to explore systemic inequities.

At some point, the district changed their reporting process to instead use a school-level dashboard. While this increases accessibility for those who do not work with data, it also limits the flexibilty for those who want to analyze the data themselves. In addition, this means that the PDFs we used in the first edition are no longer available on the district website. This is a common occurance in education. We cannot always assume that the data that we have been using 

The 2018 district data was provided in PDF format and shared on the district website. We will show how to import the PDF into R, clean the data, and then visualize and analyze the results.
...
As usual, we begin our script by calling the packages we will use. If you have not installed any of these packages yet, see the ["Packages" section](#c06p) of the ["Foundational Skills"](#c06) chapter. Load the packages, as they must be loaded each time you start a new project.
...
If you would like to follow the data cleaning steps below, you have three options of getting the data from the PDFs;

1. We can use {tabulapdf}, which pulls the PDF tables into lists using `extract_tables()`. The PDFs are available via the Wayback Machine and in the book's [Github repository](https://github.com/data-edu/data-science-in-education/tree/master/data/agg_data) (https[]()://github.com/data-edu/data-science-in-education/tree/master/data/agg_data). Then, you can follow the data workflow below to clean and process the data.
2. We can get the imported but unprocessed data from the book's [Github repository](https://github.com/data-edu/data-science-in-education/tree/master/data/agg_data) (https[]()://github.com/data-edu/data-science-in-education/tree/master/data/agg_data). They are available in the files `race_pdf.Rds` and `frpl_pdf.Rds`. If you would like to set up the folders in your working directory in the same way they are in the book, first create a folder called `"data"`. Then, inside that folder, create a second folder called `"agg_data"` and place the `race_pdf.Rds` and `frpl_pdf.Rds` files in the `"agg_data"` folder. Then, you can load the data using `here()`. Otherwise, you will have to change the file path inside of `here()` to match where the data is stored on your working directory. 
3. Finally, you can get the imported but unprocessed data from the {dataedu} package.

```{r, eval = FALSE}
# Get data using {tabulizer}
race_pdf <-
  extract_tables("https://web.archive.org/web/20201112024712/https://studentaccounting.mpls.k12.mn.us/uploads/mps_fall2018_racial_ethnic_by_school_by_grade.pdf")
...
We then transform the list to a data frame by, first, making the matrix version of the PDFs into a tibble by using `map(as_tibble())` from the {purrr} package. Then, we use the `map_df()` function then turns these tibbles into a single data frame. The `slice()` inside of `map_df()` removes unnecessary rows from the tibbles. Finally, we create readable column names using `set_names()` (otherwise, they look like `...1`, `...2`, etc.).
...
# Get data using {tabulizer}
frpl_pdf <-
  extract_tables("https://web.archive.org/web/20220307174824/https://studentaccounting.mpls.k12.mn.us/uploads/fall_2018_meal_eligiblity_official.pdf")
...
Similar to the Race/Ethnicity PDF, we take the PDF matrix output, turn it into tibbles, then create a single data frame. We remove rows that we don't need from each page using `slice()`. Then, we create column names that are easily understood.
...
To clean the dataset up further, we remove the rows that are blank. When looking at the PDF, we notice that there are aggregations inserted into the table that are not
...
When we look at these data, the district seems very diverse. Almost 40% of students are Black, and around 36% are White. Note that this matches the percentages provided in the original PDFs. This shows that our calculations above were accurate. Hooray!
...
the average demographics across the district and see what it looks like at
the school level. These distinct but closely related views demonstrate that:

1. There exists a distribution of
race/ethnicity within schools that is not representative of the district.
