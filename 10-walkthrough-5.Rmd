...
Aggregated data can tell us many things, but in order for us to examine subgroups and their information, we must have data _disaggregated_ by the subgroups we hope to analyze. This data is still aggregated from row-level data but provides data on smaller components. Common disaggregations for students include race/ethnicity, socioeconomic status, English learner designation, and whether they are served under the Individuals with Disabilities Education Act (IDEA).

Disaggregated data is essential in identifying where solutions are needed to solve disparities in opportunity, resources, and treatment. If only aggregate data is provided, we are unable to distinguish how different groups of students are doing. Disaggregated data allows us to monitor equity in educational resources and outcomes.

## Data

There are many publicly available aggregate datasets. On the international level, perhaps the most well known is:
