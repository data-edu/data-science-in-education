...
It looks like we have `r format(nrow(dat),big.mark=",",scientific=FALSE)` observations from 22 variables.
...
`r nrow(dat)`. Now that our data is ready to go, we can start to ask
...
```{r fig7-1, warning = FALSE, message = FALSE, results = 'hide', fig.showtext = TRUE, fig.cap = "Example Plot", fig.alt="Bar chart comparing school ID and mean score with School A having the lowest mean score, School B in the middle, and School C having the highest"}
# Create dataset
students <- 
  tibble(
    school_id = c("a", "b", "c"), 
...
```{r fig7-2, message = FALSE, warning = FALSE, results = 'hide', fig.cap = "Percentage Earned vs. Time Spent", fig.alt="Scatterplot of Time Spent versus Final Grade with heavy concentration of dots on low time spent and high final grades", fig.showtext = TRUE}
dat %>%
