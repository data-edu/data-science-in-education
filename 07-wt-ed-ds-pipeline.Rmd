...
Now that we've successfully loaded all three log-trace datasets, we can visually inspect the data by typing their assigned names. Running each line individually will display the first few rows (or cases/observations) of each dataset.
...
We loaded three datasets earlier in the chapter: `pre_survey`, `course_data`, and `course_minutes`. Next, we can process the course data to create new variables for analysis.

The `CourseSectionOrigID` variable stores information about the course subject, semester, and section in a single column. This format of data storage is not ideal. If we instead give each piece of information its own column, we'll have more opportunities for later analysis. We can use a function called `separate()` from the {tidyr} package to split them into distinct columns. Below, we load `course_data` and run `separate()` to extract the subject, semester, and section for easier use later on.
...
After running the code above, take a look at the `course_data` dataset to confirm it looks as expected. We should have three new variables, increasing the total number of variables from 8 to 11. The original variable `CourseSectionOrigID` is still present in the data, as well.

### Joining the Data

In this chapter, we are working with two datasets that are derived from the same courses. In order for these datasets to be most useful, we want to combine them into one dataset.

To join the `course_data` and `pre_survey` data, we need to create a common key. Our goal is to have one variable that matches across both datasets. Once we have that common variable in both datasets, we can merge the datasets on the basis of that variable.

When we look at the `course_data` and `pre_survey` datasets in our environment, we see that both have variables for courses and students. However, they have different variable names in each dataset. Our first step is to rename these variables in each of our datasets so that they match. We are not changing anything in the data other than the column headers, so that the column names align.

Let's start with the `pre_survey` data. We rename `RespondentID` to `student_id` and `opdata_CourseID` to `course_id` using the `rename()` function we learned earlier in this chapter.
...
When we look at the `student_id` variable more closely, though, we notice that there is another issue. The variable has some extra characters before and after the actual ID. Why does this variable have these additional characters? Why is there a "1" at the end of every 5-digit ID number? We are not sure! Sometimes, educational data from different systems (or used for different purposes) include additional metadata. In any event, we'll need to clean `student_id` by removing these unnecessary characters. Here
is what the variable looks like before processing:
...
We can use the `str_sub()` function from the {stringr} package. This function lets us subset string variables, that is, variables that store text data. You specify the starting and ending character positions of the string to extract.
 
Here, for example, we can use `str_sub()` to skip the first underscore and capture only the relevant portion of the string. This next chunk of code will not modify our dataset, but demonstrates how the `str_sub()` function works using a sample string formatted like our data.

```{r}
str_sub("_99888_1", start = 2)
```

We can apply the same function to remove characters from the end of a string. We use a dash (`-`) to indicate that we want to start from the right side of the string of characters. Interestingly, when we specify the argument `end = -3` below, we tell R to keep everything up to the third-to-last character. We end up removing only the last 2 characters. Our new rightmost character will be the final `8`.

```{r}
str_sub("_99888_1", end = -3)
```

Putting the pieces together, the following code should extract the 5-digit ID number we need.
...
We can apply this process to our data using `str_sub()` within `mutate()`. We convert the string into a number using `as.numeric()` in the next portion of the code. This step is
important so the data can be joined to the numeric `student_id` variables in the other datasets.
...
Now that the `student_id` and `course_id` variables are ready to go in the `pre_survey` dataset, let's proceed to `course_data.` Our goal is to rename the two variables that correspond to the course and the student so that we can match with the other variables we just created for the `pre_survey` data. In the code chunk below, we will rename both those variables.
...
Now that `course_id` and `student_id` match across both datasets, we can join them using the {dplyr} function, `left_join()`.

Note the order of the data frames passed to our "left" join. It is named based on the direction that the data is being joined. Left joins retain all of the rows in the left (first) data frame and adds every matching row in the right (second) data frame. We use key variables to join the datasets specified by `by`.

Let's save our joined data in a new object called `dat`.
...
Let's hone in on how this code is structured. After `left_join()`, we see `course_data` and then `pre_survey`. In this case, `course_data` is the "left" data frame (passed as the first argument), while `pre_survey` is the "right" data frame (passed as the second argument). So, in the above code, what happens? You can run the code yourself to check.

Our aim with that code is that all of the rows in `course_data` are retained in our new data frame, `dat`, with matching rows of `pre_survey` joined to it. An important note is that there are not multiple matching rows of `pre_survey`; otherwise, you would end up with more rows in `dat` than expected. There is a lot packed into this one function. Joins are extremely powerful - and common - in many data analysis processing pipelines, both in education and in any field. Think of all of the times you have data in more than one data frame, but you want everything to be in a single data frame! As a result, we think that joins are well worth investing the time to be able to use.

With most types of data, `left_join()` is helpful for carrying out most tasks related to joining datasets. However, there are functions for other types of joins that we want to make sure you know how to use. They may be less frequently used than `left_join()`, but they are still worth mentioning. Note that for all of these, the "left" data frame is always the first argument, and the "right" data frame is always the second. When running the code chunks below, it can be helpful to pay attention to the number of observations and variables in the datasets before and after the joining. Eventually, the obscure names of these types of joins will start to become more intuitive as you use them more often.

#### `semi_join()`

The `semi_join()` function joins and retains all of the matching rows in the "left" and
"right" data frame. This is useful when you are only interested in keeping the rows that are able to be joined. `semi_join()` will not create duplicate rows of the left data frame, even when it finds multiple matches on the right data frame. It will also keep only the columns from the left data frame.

For example, the following returns only the rows that are present in both `course_data` and `pre_survey`, and all rows in `course_data`:
...
The `right_join()` function works the same as `left_join()`, but retains all of the rows in the "right" data frame, and joining matching rows in the "left" data frame (so, the opposite of `left_join()`).
...
If we wanted this to return exactly the same output as `left_join()` (and so to create a data frame that is identical to the `dat` data frame we created above), we could switch the order of the two data frames to be the opposite of those used for the `left_join()` above:
...
Now that we've gone through the different types of joins available, we will return to our main focus: joining our course datasets together. While we didn't do any data processing steps on it, we still have the `course_minutes` dataset in our environment from when we loaded it there earlier in the chapter. In the code chunk below, we will rename the key variables to match `dat`. Then, we will merge the `course_minutes` dataset with its newly renamed variables `student_id` and `course_id`, with our `dat` dataset.
...
Note that they're now combined, even though the course data had many more rows. The pre-survey data has been joined for each student-course combination. We have a pretty large data frame! Let's take a quick look.
...
If a student was enrolled in two courses, she will have a different final grade for each course. However, our data in its current form has many rows representing each course. An quick way we can visually inspect to make sure each row correctly represents a unique student-course combination to use the `glimpse()` function. Try it below.

```{r}
glimpse(dat)
```

You can also use `View(dat)` to explore the data in RStudio's viewer.

Visually inspecting the first several rows of data, we see that they all correspond to the same student for the same course. As we expected, the `FinalGradeCEMs` variable (representing students' final grade) is also consistent across these rows. 

Since we are not analyzing individual `Gradebook_Item` values, these duplicate rows are not necessary. We only want variables at the student level, not at the gradebook level. We can extract only the unique student-level data using the `distinct()` function.

Imagine having a bucket of Halloween candy that has 100 pieces of candy. You know that these 100 pieces are really just a bunch of duplicate pieces from a relatively short list of candy brands. `distinct()` takes that bucket of 100 pieces and returns a bucket containing only one of each brand.

By default, `distinct()` will only return the variable(s) (it is possible to pass more than one variable to `distinct()`) you used to determine uniqueness, unless you include the `.keep_all = TRUE` argument. For the sake of simplicity, we omit this argument for now.

Were we to run `distinct(dat, Gradebook_Item)`, what do you think would be returned? Running the following code returns a one-column data frame that lists the names of all unique gradebook items.
 
```{r}
distinct(dat, Gradebook_Item)
```

You might be wondering whether some gradebook items share the same names across different courses. We can find the unique combination of courses and gradebook items by adding another variable to `distinct()`:
...
This is a much smaller data frame, with just one row for each student in the course. The previous version of `dat` had over 40,000 rows, useful for analyzing specific gradebook items. However, since we are focusing on student-level data, our new dataset keeps only the
unique combinations of student and course, leaving us with a more manageable number of observations: `r nrow(dat)`. Now that our data are ready to go, we can start to ask some questions of the data.

Let's take one last step. Since we use the final grade variable `FinalGradeCEMS` in many of the figures and analyses that follow, let's rename it using`rename()` to something that is a bit easier to type and remember:
