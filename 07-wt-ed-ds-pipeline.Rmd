...
code more efficiently, as you will recall from Chapter INTRODUCTORY. We load
...
If we wanted this to return exacty the same output as `left_join()` (and so to create a data frame that is identical to the `dat` data frame above), we could simply switch the order of the two data frames to be the opposite of those used for the `left_join()` above:
