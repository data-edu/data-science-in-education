...
correspond to each category: interst, utility value, and perceived competence.
...
questions 3 and 7 to calcluate `pc`. We indicate the start of each list with the
...
The pre\_survey data has been joined for each student-course combination. We
...
There is one last step to take. If we were interested in a fine-grained analysis
of how students performed on different teacher-graded assignments (see the
`Gradebook_Item` column), we would keep all rows of the data. But, our goal is
more modest: to calculate the percentage of points students earned as a measure
of their final grade. Though we have chosen this target for analysis, we
acknowledge that the teacher may have assigned a different final grade than the
total points would indicate. It's also possible the teacher could have weighted
their grades in ways not reflected through the points. Even so, we'll calculate
this variable below to illustrate one educational outcome we might be interested
in examining.

In the code chunk below, we carry out a number of steps all at once. We group
our merged dataframe by `student_id` and `course_id`. We transform the existing
variable `Points_Earned` into integer format. Then, we create two new variables
that sum the number of total points possible and total points earned,
respectively: `total_points_possible` and `total_points_earned`. We create a new
variable using `mutate()` that takes those two new variables and calculates a
percentage from them, and we call the resulting variable `percentage_earned`. We
ungroup the data so that we are not sectioning it out by student and course
anymore. Finally, we re-join the data back with the original dataframe to retain
all the newly created variables and all the previously existing variables.
...
This last step calculated a new column for the percentage of points each student
earned. Since we grouped our data by both `student_id` and `course_id`, we
calculated the percentage for each of those unique combinations. If a student
was enrolled in two courses, she will have a different percentage for each of
those two courses. However, since there are many gradebook items, our data in
its current form has many rows representing EACH course. Therefore, the new
variable `percentage_earned` will be the same across all rows for the same
student, by course. An easy way we can visually inspect to make sure every row
is the same for the same student, by course is to use the `View()` function. Try
it below.
...
`percentage_earned` variable is also consistent across these rows. Since we are
not carrying out a finer-grained analysis using the `Gradebook_Item`, these
...
Running the code above retruns a one-column dataframe that lists the names of
every distinct gradebook item. You might be wondering whether some gradebook
...
course, the more points they earned.
...
a student to earn. Percentage earned is the dependent, or *y*-variable, and so
...
deviation increase in the amount of time spent on the course, the percentage of
points a student earns increases by .11, or 11 percentage points.
