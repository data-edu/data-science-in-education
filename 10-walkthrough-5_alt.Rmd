---
title: "Exploring Aggregate Data"
...
Federal and international education datasets provide additional context for evaluating education systems. In the US, some federal datasets aim to consolidate important metrics from all states. This can be quite useful because each state has its own repository of data and to go through each state to download a particular metric is a significant effort. However, the federal datasets often have more stringent data requirements than the states, so the datasets may be less usable. The federal government also funds assessments and surveys which are disseminated to the public.
...
1. `tabulizer` easily and quickly pulls the district data into lists using `extract_tables()`.
...
- Removing unnecessary or blank columns using `select()`. Negative selections means those columns will be removed.
2. Removing all grand total rows (otherwise they'd be redundant) using `filter()`. We keep schools that have "Total" in the name but remove any rows that are Grand Total.
...
We will do the same to the Free Reduced Price Lunch (FRPL) PDF's.

> FRPL stands for Free/Reduced Price Lunch, often used as a proxy for poverty. Students from a household with an income up to 185 percent of the poverty threshold are eligible for free or reduced price lunch. (Sidenote: Definitions are very important in disaggregated data. FRPL is used because it’s ubiquitous and reporting is mandated but there is [debate as to whether it actually reflects the level of poverty among students.](https://nces.ed.gov/blogs/nces/post/free-or-reduced-price-lunch-a-proxy-for-poverty))
...
frpl_filter <-
  frpl_df %>% 
  filter(school_name != "") %>% # remove blank school names
...
1. We create the `frpl_pct` by adding the number of students who receive free lunch and those who receive reduced price lunch, then dividing by number of total students.
2. We want to calculate, for each race, the number of students in 'high poverty' schools. This is defined by NCES as schools that are over 75% FRPL. When a school has over 75% FRPL, we count the number of students for that particular race under the variable `[racename]_povnum`.
3. The `janitor` package has a handy `adorn_totals()` function that sums the columns for you. This is important because we want a weighted average of students in each category, so we need the total number of students in each group.
4. We create the weighted average of the percentage of each race by dividing the number of students by race by the total number of students.
5. To get FRPL percentage for all schools, we have to recalculate `frpl_pct.`
6. To calculate the percentage of students by race who are in high poverty schools, we must divide the number of students in high poverty schools by the total number of students in that race.

```{r}
merged_df <-
    merged_df %>%
  mutate(frpl_pct = (free_num + reduce_num)/total_students, #1
         hi_povnum = case_when(frpl_pct > .75 ~ hi_num), #2
  ...
  mutate(na_pct = na_num/tot, #4
         ...
         hi_pct = hi_num/tot,
         wh_pct = wh_num/tot,
         frpl_pct = (free_num + reduce_num)/total_students, #5
         hi_povsch = hi_povnum/hi_num[which(school_name == "Total")], #6
...
When we look at these data, the district looks very diverse. Almost *40% of students are Black* and around *35% are White.*
...
*60% of the students are eligible for FRPL*, which is [close to the US average of 52.1%.](https://nces.ed.gov/programs/digest/d17/tables/dt17_204.10.asp?current=yes)

Now, we dig deeper to see if there is more to the story.

## Analyzing spread

Another view of the data can be visualizing the distribution of percentage of a demographic within schools. Here is a histogram for the percentage of White students within the schools for which we have data.

```{r}
# hist(merged_df$wh_pct)$counts[1] # counting number of schools in the first bin (0-10%)
...
*9% of White students* attend high poverty schools, compared to *46% of Black students, 51% of Hispanic students, 46% of Asian students, and 49% of Native American students.* We can conclude these students are disproportionally attending high poverty schools.
