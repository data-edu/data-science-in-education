...
outstanding introduction to their use exists in Bryan's freely-available [Happy git with R](https://happygitwithr.com/).

- Building R packages: If you are carrying out the same analyses many times, it may be helpful to create your own package, such as the roomba (for tidying complex, nested lists) and tidyLPA (for carrying out Latent Profile Analysis) packages that authors of this book created. However, building an R package is not the focus of this book, and Wickham has made the helpful [R packages](http://r-pkgs.had.co.nz/) book freely-available
...
- Creating a website (or book): While this book is created using the bookdown package (for creating a book through R), and many of us have created our personal websites using the blogdown R package (for cerating a website through R), this book does not describe how to do these; there are excellent, freely available books (see books for [blogdown](https://bookdown.org/yihui/blogdown/) and [bookdown](https://bookdown.org/yihui/bookdown/) books).
...
built ways for you to share with us. Here's how you can can contribute:
  ...
  - Submit a pull request to our [GitHub
    site](https://github.com/jrosen48/data-science-in-education) that describes
