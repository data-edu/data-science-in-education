...
Supervised machine learning can use the very same statistical model --- including the logistic regression you'll use in this walkthrough --- but with a different aim. In an SML approach, the interesting output isn't the coefficients; it's the *predictions* themselves. Rather than asking "how does motivation relate to grades?", you'll ask "how accurately can we predict each student's grade?"

Consider the familiar regression equation:

$$y = b_0 + b_1x_1 + b_2x_2 + \cdots + e$$

If your goal is inference --- explaining how variables relate to an outcome --- you'll typically use theory and prior research to choose a small set of predictors whose individual effects you can interpret. You'll favor a *transparent* model where you understand how each predictor relates to $y$.
...
Other times, you may use _cross-validation_, where you'll split the training data many times and fit the model to each of these splits. We'll return to cross-validation briefly in the Conclusion. For more on the technique, consider reading Chapter 2 in [@boehmke2019hands].
...
**Further reading and practice.** A great free, applied resource is *Tidy Modeling with R* by Max Kuhn and Julia Silge (available at <https://www.tmwr.org/>), which covers the full {tidymodels} workflow including resampling, tuning, and many model types. For a deeper conceptual treatment with additional case studies in education, the LASER Institute's supervised machine learning materials are available at <https://github.com/laser-institute/supervised-machine-learning>.
