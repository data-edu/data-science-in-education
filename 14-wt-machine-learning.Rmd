# Walkthrough 8: Predicting Students' Final Grades Using Machine Learning Methods
...
data <- data %>% filter(semester != "S217")

#selecting only the variables we are interested in: 
data <- data %>% 
    select(pre_int, pre_uv,  pre_percomp, time_spent, final_grade, subject, enrollment_reason, semester, enrollment_status, cogproc, social, posemo, negemo, n)
```
...
data <- select(data, -enrollment_status)
```
...
data_train <- filter(data, temp_id %in% trainIndex)
...
data_test <- filter(data, !temp_id %in% trainIndex)
...
data <- select(data, -temp_id)
data_train <- select(data_train, - temp_id)
data_test <- select(data_test, -temp_id)
```
...
tune_grid <- expand.grid(mtry = c(2, 3, 7, 10, 19),
...
data_test_augmented <- mutate(data_test,
                           pred = predict(rf_fit2, data_test),
                           obs = final_grade)
...
data_train_lm <- mutate_if(data_train, is.character, as.factor) 
