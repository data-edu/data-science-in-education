...
We can find out exactly what the relationship is using a linear model. A full exploration of linear models is outside the scope of this book, but we do we want to discuss it briefly since we'll be using the technique a few times in later chapters. A linear model is an equation that describes the relationship between two variables that have a linear shape when you graph them on a scatter plot. Here's an example using the `mtcars` dataset, which shows that as engine volume (`disp`) goes up, fuel efficiency (`mpg`) goes down in roughly the shape of a straight line:
