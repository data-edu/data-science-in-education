...
One of the most important steps is to process and prepare data. This walk through starts here instead of loading data because of the importance of these steps and because they're exciting places to begin: you can quickly transform an unruly data set into one ready to be plotted or modeled. 
...
And now we can load a file. 


We are going to call the data `student_responses`:

```{r, eval = F}
student_responses <- read_csv(here::here("data/")) # need to add file
```
...
We can then use `read_excel()` in the same way as `read_csv()`:
...
This section focuses on two types of models, *t*-tests and linear models (for regressions).
...
Then, click "Knit to PDF" or "Knit to Word".
