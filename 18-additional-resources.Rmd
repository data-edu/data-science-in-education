...
> `dplyr` is a R package for data manipulation. It consists of several "vebs", that are essentially functions which relate to data manipulation activities. The dplyr verbs are easy to use and the idea of using them is to ensure code readability as well as easing the data analysis task. Anyone, with little to zero programming expertise can easily comprehend the dplyr verbs to use them in their analytical workflow with comparative ease.
