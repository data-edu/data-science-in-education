...
> This book provides over 200 practical recipes for analysing data using R. 
...
> The `caret` package is an abbreviated form of `Classification And REgression Trees`. It contains functions to streamline the model training process for complex regression and classification problems. The idea behind this package is to streamline the process for creating predictive models. The package contains tools for:
...
> The philosophy of ensuring data is clean such that each column contains data of just one type (or atomicity) and each row is an observation (made up of several atomic columns) is made possible in this paper. The principles of tidy data provide a standard way to organise data values within a dataset.
