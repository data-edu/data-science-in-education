...
Even if the only person you ever collaborate with is a future version of yourself, using a Project for each of your analyses will mean that you can move the Project folder around on your computer and remain confident that the analysis will run in the future.
...
Many functions in R packages do not require you to pass them arguments. They use a set of default arguments unless you provide something different. There are no hard and fast rules about when a function needs an argument. However, if you are having trouble running your code, check the Help documentation to see if you can provide arguments that more clearly direct R what to do.
...
While it is entirely possible to do your work in R without packages, it's not recommended. There are a wealth of packages available that reduce the learning curve and the time spent on analytical projects. 
...
In order to access the functions within a package, you must first install the package on your computer. There is a collection of R packages hosted on the internet on the CRAN website: [CRAN](https://cran.r-project.org/)(https:[]()//cran.r-project.org/), the Comprehensive R Archive Network. These packages must meet certain quality standards, and are regularly tested.
...
The {dataedu} package is not available on CRAN yet. You'll be installing it from GitHub. To do this, you first need to install the {pak} package.
...
This will print a list of the packages included in the {dataedu} package. You can then install each package individually using `install.packages("package_name")`.

If you encounter errors, please reach out to us! You can file an issue on GitHub, or email us at [authors@datascienceineducation.com](mailto:authors@datascienceineducation.com).
...
You'll practice doing this in a later section of this chapter.
...
Within RStudio, access the "Help" documentation by using `?` or `??` in the Console. For example, if you wanted to look up information on the `data()` function, type `?data` or `?data()` next to the `>` prompt in the Console, and hit `Enter`. 

Try this now. You should see the `Help` panel in your RStudio environment display documentation on the `data()` function.

This works because the `data()` function is part of "base R"---it's included with R when you first install it. But this also works for packages outside of base R. 
...
```{r fig6-2, fig.cap = "Example from the `coalesce()` help documentation", echo = FALSE}
...
Testing limits helps you recognize patterns, develop a hypothesis about what code does, and test whether that hypothesis is true.
...
Before beginning this section, you'll need to have installed the {dataedu} package and to have run `dataedu::install_dataedu()` to install the associated packages. If you have not done this yet, please do so before continuing.
...
```{r fig6-3, fig.cap = "List of attached packages and associated conflicts when loading the tidyverse", echo = FALSE}
...
```{r fig6-4, fig.cap = "Loading the `ma_data` dataset", echo = FALSE}
...
- How do the results in the Console change with each line of code you run?
...
Before a fuller explanation of the code below, let's discuss the error. You got an error due to an "unexpected symbol". Like the earlier examples, this error is caused by the space in the variable name. In the code chunk you just ran, you can enclose `District Name` in backticks to resolve this error.
...
```{r fig6-5, fig.cap = "The Treachery of Images by Magritte", echo = FALSE}
...
Naming things is important! The more you use R, the more you'll develop a sense of how you prefer to name things, either as an organization or an individual programmer. However, R has some hard and fast rules about naming things. Using the code chunk below, try saving the `ma_data_init` dataset into a few different object names. You'll be using the `clean_names()` function from the {janitor} package, which you already loaded into your environment earlier using the `library(janitor)` function. Type out and run the following lines of code one by one, and notice what happens in the Console after you run each line.
