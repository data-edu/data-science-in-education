...
In [Chapter 5](#c05), you might have noticed at the very end that we installed two packages ({pak} and {dataedu}), but we didn't talk too much about what we were doing. 
...
[CRAN Task Views](https://cran.r-project.org/web/views/) (https[]()://cran.r-project.org/web/views/) is one of the best resources for seeing what packages are available and might be relevant to your work.
Other great resources to learn about various R packages are through Bluesky (following the "#rstats" hashtag) as well as through Google searches.
...
The first function, `install.packages("pak", repos = "http://cran.us.r-project.org")`, has two arguments, `"pak"` and `repos = "http://cran.us.r-project.org"`.
The first argument, `"pak"`, is the name of the package we want to install.
...
In order for us to load and use the {dataedu} package, we first needed to install the {pak} package.

The second function, `pak::pak("data-edu/dataedu")`, has only one argument, `"data-edu/dataedu"`, but also looks a little different from the first function.
What we're doing here is telling R to go to the {pak} package to find the `pak()` function.
The `pak()` function is telling R to go to a specific repository on GitHub to get the code for the {dataedu} package.
...
For those curious about how to accomplish this, we recommend starting with the [Best Practices in Working with Databases](https://solutions.posit.co/connections/db/) (https[]()://solutions.posit.co/connections/db/) resource from Posit.
