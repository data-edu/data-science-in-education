...
number of students with disabilities in a given year. First, you'll review how to get the `.csv` files you'll need for the task. Alternatively, we make these data files available through the {dataedu} package. 

Note that if you use this walkthrough on different datasets or store datasetsin different locations on your computer, you'll need to your code accordingly. We suggest only doing this if you have previous experience using R.
...
in our working directory: "data/longitudinal\_data". you'll be using the `here()` *function* from the {here}
...
when referencing the file path "data/longitudinal\_data", you'll use code like
this:
...
function. Let's try it first with one dataset, then you'll scale our solution up
...
find out, you'll use this strategy:
...
you'll correct this in the next section by selecting and renaming our variables,
...
Let's preview the steps you'll be taking:
...
variable name row. To fix it, you'll read the 2016 dataset again using
`read_csv()` and the fifth element of `filenames` but this time will use the
argument `skip = 3`. you'll assign the newly read dataset to the fifth element of
the `all_files` list:
...
trial and error, but here is what we found we needed:
  ...
  - Later, you'll need to filter our dataset by disability category and program
    location so you'll want `SEA Education Environment` and `SEA Disability Category`
  - We want to make comparisons by state and reporting year, so you'll also pick
    `State Name` and `Year`
...
Now you'll turn our attention to combining the datasets in our list `all_files`
into one. you'll use `bind_rows()`, which combines datasets by adding each one to
...
the dataset, you'll filter out all subgroups except "All Disabilities" and the
age totals:
...
In the next section you'll prepare the dataset for visualization and modeling by
"tidying" it. When we write code to transform datasets, you'll be typing the
...
To make the values of the `gender` column more intuitive, you'll use
...
numeric columns. you'll use `mutate()` to change the count columns. 
...
will ask, "How have child counts changed over time?" First, you'll use
visualization to explore the number of students in special education over time.
In particular, you'll compare the count of male and female students. Next, you'll
use what we learn from our visualizations to quantify any differences that we
see.

### Visualize the dataset

Showing this many states in a plot can be overwhelming, so to start you'll make a
...
`filter()`. you'll call this new dataset `high_count`:
...
In that spirit, you'll start by visualizing specific genders and age groups. Feel
...
It's worth calling out one more thing; since it's a technique you'll be using as
...
visualized the total amount of students each year per state? To do that, you'll
need to add both gender values together and both age group values together.
you'll do this using a very common combination of functions: `group_by()` and
`summarize()`. 
...
you'll explore ways to quantify the relationship between male student counts and
...
In this example, you'll follow our intuition about the relationship between male
and female student counts in our special education dataset. In particular, you'll
