...
Data scientists working in education don't always have access to student level data, so knowing how to model publicly available datasets, as in [the previous chapter](#c9), is a useful skill. This walkthrough builds upon and extend the focus on aggregate data in the last chapter to focus on change over time in students with disabilities in each state.

### Background 

In this chapter, we'll be learning some ways to explore data over time. In addition, we'll be learning some additional techniques for exploring a publicly available dataset. Like most public datasets (see [the previous chapter](#c9)), this one contains aggregate data. This means that someone totaled up the student counts so it doesn't reveal any private information. 

You can download the datasets for this walkthrough on the United States Department of Education website (see @usdoe2019)^[The documentation for the dataset is available here: https://www2.ed.gov/programs/osepidea/618-data/collection-documentation/data-documentation-files/part-b/child-count-and-educational-environment/idea-partb-childcountandedenvironment-2017-18.pdf], though they are also available in the {dataedu} package that accompanies this book.
...
```{r total female students over time, results = 'hide', message = FALSE, warning = F}
high_count %>%
...
```{r total male students over time, results = 'hide', message = FALSE}
high_count %>%
...
```{r total students over time, results = 'hide', message = FALSE}
high_count %>%
  group_by(year, state) %>%
...
```{r median total per state, results = 'hide', message = FALSE}
high_count %>%
  group_by(year, state) %>%
  summarize(n = sum(total)) %>%
...
```{r male to female ratio over time, results = 'hide', message = FALSE}
high_count %>%
...
```{r plot female students to male students, results='hide', message = FALSE}
child_counts %>%
...
```{r plot without outliers, results = 'hide', message = FALSE}
child_counts %>%
  filter(age == "Total, Age 6-21") %>%
  pivot_wider(names_from = gender, 
              values_from = total) %>%
...
```{r ratios across years, results = 'hide', message = FALSE}
...
```{r visualize female to male ratio, results = 'hide', message = FALSE}
model_data %>%
