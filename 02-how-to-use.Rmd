...
- Building R packages: If you are carrying out the same analyses many times, it may be helpful to create your own package. Packages are collections of code and sometimes data, such as the {roomba} (for tidying complex, nested lists) and {tidyLPA} (for carrying out Latent Profile Analysis) packages that authors of this book created. However, building an R package is not the focus of this book. Hadley Wickham and Jenny Bryan wrote a very helpful---and freely available---book on the topic called [*R Packages*](https://r-pkgs.org/) (https://r-pkgs.org/) [@rpackages].
...
One question we asked ourselves was: how do we expand this work as data science in education expands as a field? We want readers of this book to be equipped with an agile skillset, and we want this book to continue to provide that even as new R packages are developed and new methods arise. In this edition, we've updated our examples, refreshed the packages, and followed the latest best practices so you have access to the latest tools while preserving the theme of continuous learning and growth.
