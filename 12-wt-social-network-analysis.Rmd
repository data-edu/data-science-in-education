...
For instance, social network analyses can be useful for asking questions about _social influence_ [@frank1998chapter]:
...
We can also ask questions about _social selection_, 
...
### Alternative approach: Using regular expressions to extract mentions

The code below shows an alternative (older) approach to extracting mentions using regular expressions. This approach is kept here for reference, but the method above (using the structured data from Bluesky's API) is more reliable and comprehensive.
...
**(Old approach continued)** Putting the edgelist together

The alternative regex-based approach would continue by creating an edgelist structure. However, this approach has limitations compared to using the structured API data we extracted above.
