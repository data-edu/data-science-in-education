...
This chapter explores the use of social network analysis (commonly referred to as SNA). While much of the data that data scientists in education analyze is about individuals, some data concerns the relationships between individuals, such as friendship between youth, or advice-seeking on the part of teachers. This is called network data. Network data can be difficult to analyze, in part due to the multiple sources of data (about both individuals’ relations and individuals and the complexity of the relations between individuals. This chapter uses data from the Bluesky #tidyuesday network, an R learning community, to demonstrate how such data can be accessed and imported, processed into forms necessary for social network analysis using the {tidygraph} R package, and visualized using the {ggraph} R package.
...
In education, we are used to focusing on *individuals* – students and teachers, especially. We can learn many things from studying individual students or teachers, but, sometimes, what is interesting lies *between* individuals. Social network analysis is useful for just this – understanding the relationships between individuals.

For instance, social network analyses can be useful for asking questions about *social influence* [@frank1998chapter], such as, how does the pattern of interactions among teachers in a professional network explain their continued involvement in the network? [@rosenberg2021advancing]

We can also ask questions about *social selection*, such as, which parents are the most central in a parent teacher association (PTA) group?

In this chapter, we focus on the *interactions* between #TidyTuesday participants using social network analysis (sometimes simply referred to as network analysis) techniques. #TidyTuesday is a weekly social data project aimed at the R community. Every Monday, a new dataset is posted on social media, and participants create and share visualizations throughout the week, creating a vibrant online learning community.

While useful for asking questions about social relations, social network analyses can be hard. Cleaning and tidying the data, especially, can be tricky. It can be even more challenging than for most other data sources because data for social network analysis (or network data) often includes variables about both individuals (such as information about students or teachers) and their relationships (whether they have a relationship at all, for example, or how strong or of what type their relationship is).
...
Traditionally, social network analysis was carried out using self-report surveys. An example of this is @frank2004social.

More recently, researchers and analysts have begun other data sources---especially data from digital tools, including social media [@rosenberg2020idle].
...
The first step in the use of the {bskyr} package is to gain access to the Application Programming Interface.
...
![This is the name for your "app password" - you can use the default generated name or provide your own that is specific to your analysis or use.](images/Screenshot%202026-01-05%20at%201.55.05%20PM.png)
...
![Example app password (note, this password won't work for you - you'll need to create your own!).](images/Screenshot%202026-01-05%20at%201.53.30%20PM.png)
...
For this walkthrough, we'll use pre-collected data that we've saved in the file `posts.rds`. This allows you to work through the analysis even if you don't have a Bluesky account or want to avoid API rate limits. However, the code above shows you how to collect fresh data yourself for your own network analyses.
...
In a regular spreadsheet, each cell contains a single value (like "John" or "42"). But in a named list, a single cell can contain *multiple* pieces of related information. For the author, this includes their username, display name, profile information, and more—all stored together in one complex cell. Take a look at one author's information:
...
Before we dive into extracting data, let's be clear about what our network represents. Recall the discussion at the beginning of this chapter about the wide variety in types of networks. In this analysis, we're creating a network where:

- **Nodes** (the dots) represent individual Bluesky users who participated in #tidytuesday
- **Edges** (the connections) represent interactions: when one user mentions or replies to another user
...
To build this network, we need to extract the author's unique identifier (their DID) to identify each person as a node in our network. But we can't easily use it while it's buried inside this nested list structure—that's the challenge we'll tackle next.

## View data

Let's get a sense of how much data we're working with. Each row in our `posts` dataset represents one post that used the #tidytuesday hashtag:
...
The network dataset needs a way to identify each participant's role in the interaction. We need to answer questions like: Did someone reach out to another for help? Was someone contacted by another for help? We can process the data by creating an "edgelist".

An **edgelist** is a special type of table used in network analysis where each row represents a unique interaction between two parties. Think of it like a log of phone calls: each row records who called whom. Each row (which represents a single relationship) in the edgelist is referred to as an "edge".
...
An edgelist looks *like* the following example, where the `sender` (sometimes called the "nominator") column identifies who is initiating the interaction and the `receiver` (sometimes called the "nominee") column identifies who is receiving the interaction:
...
Above, we saw that our data is really complex, and we saw what an edgelist looks like. To create an edgelist from *our* data, we need to extract two key pieces of information from `posts`:

1.  **Nodes**: The unique individuals (users) in our network
2.  **Edges**: The interactions between users (replies and mentions)

First, we'll define a helper function to safely extract data from the nested list structures in the Bluesky data. Remember that our Bluesky data has information stored in complex nested lists (like boxes within boxes), and we need a safe way to open these boxes and get the information inside:

```{r, extract-data}
# Create a helper function to extract data from nested lists
# This function "reaches into" nested list structures and pulls out text values
# If the value doesn't exist, it returns NA instead of causing an error
...
# Each row in 'posts' has an 'author' column containing nested information
# We use map_chr() to apply our pluck_chr function to each row
# The ~ syntax creates a formula, and .x represents each individual author
nodes <- posts %>%
  mutate(
...
# Extract edges (interactions between users)
# We'll look at two types of interactions: replies and mentions

# First, get reply edges (when someone replies to another user's post)
# A reply creates a connection from the person replying TO the person they're replying to
reply_edges <- posts %>%
  mutate(
...
# "Facets" are pieces of a post that have special meaning, like @mentions or #hashtags
# When you @mention someone, that creates an interaction (edge) in the network
...
# Combine both types of edges (replies and mentions) into one edgelist
# bind_rows() stacks the two data frames on top of each other
# distinct() removes any duplicate interactions
mention_edges <- bind_rows(reply_edges, mention_facet_edges) %>%
  distinct()

# Final cleaning of our nodes data
# Make sure we don't have any missing or blank IDs
nodes <- nodes %>%
...
```{r}
# Calculate centrality measures for each person in the network
# activate(nodes) tells tidygraph: "I want to work with the nodes table now"
# (Remember, our graph has both nodes and edges - we need to specify which one)
g_metrics <- g_mentions %>%
  activate(nodes) %>%
  mutate(
    # IN-DEGREE: How many times was this person mentioned or replied to?
    # This measures popularity or influence - like counting how many people
    # call you for advice
    in_deg  = centrality_degree(mode = "in"),

    # OUT-DEGREE: How many times did this person mention or reply to others?
    # This measures how active someone is in reaching out to others
    out_deg = centrality_degree(mode = "out"),

    # BETWEENNESS: How often does this person connect different groups?
    # Think of this as being a "bridge" between different friend groups
    # High betweenness means you connect people who wouldn't otherwise interact
    # normalized = TRUE puts it on a 0-1 scale for easier interpretation
    btw     = centrality_betweenness(
                directed = TRUE, normalized = TRUE
              )
  )

# Convert the network back to a regular table and add ranking columns
# This helps us identify the "top" people by each measure
nodes_tbl <- g_metrics %>%
  activate(nodes) %>%
  as_tibble() %>%    # Convert from graph format to regular data frame
  mutate(
    # Rank people by each measure (1 = highest, 2 = second highest, etc.)
    # desc() means we rank from high to low (not low to high)
    in_deg_rank  = min_rank(desc(in_deg)),   # Who gets mentioned most?
    out_deg_rank = min_rank(desc(out_deg)),  # Who mentions others most?
    btw_rank     = min_rank(desc(btw))       # Who bridges groups most?
  )
