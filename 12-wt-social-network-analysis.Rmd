...
# ---- 3) Top-15 bar charts for each metric ----
top_n_plot <- function(df, metric, title, fill_lab) {
  df %>%
    arrange(desc(.data[[metric]])) %>%
    slice_head(n = 15) %>%
    mutate(label_short = ifelse(nchar(label) > 28, paste0(substr(label, 1, 25), "…"), label)) %>%
    ggplot(aes(x = reorder(label_short, .data[[metric]]), y = .data[[metric]], fill = .data[[metric]])) +
    geom_col() +
    coord_flip() +
...
p_in  <- top_n_plot(nodes_tbl, "in_deg",  "Top 15 by In-degree (mentioned most)", "In-degree")
p_out <- top_n_plot(nodes_tbl, "out_deg", "Top 15 by Out-degree (mentions others)", "Out-degree")
p_btw <- top_n_plot(nodes_tbl, "btw",     "Top 15 by Betweenness (bridging)", "Betweenness")

print(p_in); print(p_out); print(p_btw)
...
g_no_isolates <- g_mentions %>%
  activate(nodes) %>%
  mutate(deg_all = centrality_degree(mode = "all")) %>%
  filter(deg_all > 0) %>%
  select(-deg_all)
...
# 1) Compute weak-component membership on an undirected copy
g_und <- g_mentions %>% convert(to_undirected)
mem <- igraph::components(igraph::as.igraph(g_und), mode = "weak")$membership

# 2) Attach membership to original graph and keep the largest component
g_giant <- g_mentions %>%
  activate(nodes) %>%
  mutate(comp = mem) %>%
  filter(comp == which.max(tabulate(mem))) %>%
  select(-comp)
...
ggraph(g_giant, layout = "fr") +
  geom_edge_link(alpha = 0.15, linewidth = 0.2) +
  geom_node_point(aes(size = centrality_degree(mode = "in"),
                      color = centrality_betweenness(directed = TRUE, normalized = TRUE)),
                  alpha = 0.9) +
  theme_void()
...
```{r}
posts %>% 
    select(reply_count, repost_count, like_count, quote_count) %>% 
    gather(key, val) %>% 
    ggplot(aes(x = val)) +
    geom_histogram() +
    facet_wrap(~key)
...
pluck_lgl <- function(x, ..., .default = NA) {
  purrr::pluck(x, ..., .default = .default) %>% as.logical()
}

posts_norm <- posts %>%
  mutate(
    # Author fields are usually a small record with did, handle, displayName
    author_did    = map_chr(author, ~pluck_chr(.x, "did")),
    author_handle = map_chr(author, ~pluck_chr(.x, "handle")),
    author_name   = map_chr(author, ~pluck_chr(.x, "displayName")),

    # Record-level fields (createdAt, text, langs)
    rec_type      = map_chr(record, ~pluck_chr(.x, "$type")),     # "app.bsky.feed.post"
    created_at    = map_chr(record, ~pluck_chr(.x, "createdAt")),
    text          = map_chr(record, ~pluck_chr(.x, "text")),
    langs         = map(record, ~purrr::pluck(.x, "langs", .default = character(0))) %>%
                      map_chr(~paste(.x, collapse = ",")),

    # Reply references (URIs to parent/root posts)
    reply_parent_uri = map_chr(record, ~pluck_chr(.x, "reply", "parent", "uri")),
    reply_parent_cid = map_chr(record, ~pluck_chr(.x, "reply", "parent", "cid")),
    reply_root_uri   = map_chr(record, ~pluck_chr(.x, "reply", "root", "uri")),

    # Embeds: if it's a record-embed, there is typically record$uri
    embed_type       = map_chr(record, ~pluck_chr(.x, "embed", "$type")),
    embed_record_uri = map_chr(record, ~pluck_chr(.x, "embed", "record", "uri"))
  )
