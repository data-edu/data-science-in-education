...
Traditionally, social network analysis was carried out using self-report surveys. An example of this is @frank2004social, whereby teachers were surveyed about who they turned to for advice about teaching. These data were then used to create a network of advice-seeking relationships among teachers.
...
There are two ways to walk through this chapter. One is to follow the steps that immediately follow whereby *you* will access data from Bluesky. The other is to load the already-accessed from Blueesky (by us) data via the {dataedu} R package. The choice is yours! Skip ahead to the "Loading already-accessed data" sub-section (below) if you'd prefer to access the data via the R package.
...
Let's get started. The first step in the use of the {bskyr} package is to gain access to the Application Programming Interface.. To do so, you'll need to visit <https://bsky.app/settings> in a browser.
...
The code below allows you to work through the analysis even if you don't have a Bluesky account or want to avoid API rate limits. However, the code above shows you how to collect fresh data yourself for your own network analyses.
...
First, we'll define a helper function to safely extract data from the nested list structures in the Bluesky data. Remember that our Bluesky data has information stored in complex nested lists (like boxes within boxes), and we need a safe way to open these boxes and get the information inside. This function "reaches into" nested list structures and pulls out text values. If the value doesn't exist, it returns `NA` instead of causing an error:
...
Now we'll extract the nodes, the unique users in our network. Each row in `posts` has an `author` column containing nested information. We use `map_chr()` to apply our `pluck_chr()` function to each row. The `~` syntax creates a formula, and `.x` represents each individual author:
...
Next, we'll extract edges—the replies and mentions between users, specifically.

First, let's get reply edges. When someone replies to another user's post, this creates a connection from the person replying to the person they're replying to:
...
Second, let's get mention edges from facets in the post record. "Facets" are pieces of a post that have special meaning, like @mentions or #hashtags. When you @mention someone, that creates an interaction (edge) in the network:
...
Now we'll combine both types of edges (replies and mentions) into one edgelist. The `bind_rows()` function stacks the two data frames on top of each other, and `distinct()` removes any duplicate interactions:
...
Finally, let's do some final cleaning of our nodes data to make sure we don't have any missing or blank IDs:
