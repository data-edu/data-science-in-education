...
One of the great features of data science that involves programming is the
flexibility in terms of what counts as a data source. So far we hae analyzed
data in CSV files, but that's not the only place data is stored. If we can learn
some basic techniques for analyzing text, we increase the number of places we
can find information to learn about the student experience, teacher professional
development, or educational systems.

When we think about data science in education, our mind tends to go data in
spreadsheets. But what can we learn about the learner experience from text data?
Take a moment to mentally review all the moments in your work day that you
generated or consumed text data. In education, we are surrounded by it. We do our
...
<!-- Note: wondering if this goes before or after the social network analysis walkthrough - if after, we can refer back to that chapter when we mention the dataset of tweets here. -->
...
Either type the same file name, in quotation marks (e.g., "my_file_name.csv")
...
name in quotation marks (e.g., "my_file_name.csv") within the parentheses of
`read_csv()` below.
