...
Now we can use `high_count` to do some initial exploration. Our analysis is about comparing counts of male and female students in special education, but with visualization is a great way to explore related curiosities. You may surprise yourself with what you find exploring. You may come up with more interesting hypotheses, find that your initial hypothesis requires more data transformation, or find interesting subsets of the data (as we did here with our freely associated states value in the `state` column). You can let your curiosity and intuition drive this part of the analysis. In that spirit, we'll visualize specific genders and age groups. Feel free to try these, but also try the other student groups for practice and more exploration.
...
high_count %>% 
    ...
    labs(title = "Count of female students in special education over time", 
         subtitle = "Ages 6-21. Selected states in the US", 
         x = "", 
         y = "Total students", 
