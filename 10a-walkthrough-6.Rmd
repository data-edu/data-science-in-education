...
Plotting this many states can be overwhelming, so to start we'll make a subset of the dataset. We can use a function in the `dplyr` package called `min_rank()` to help us learn which states have the highest average count of special education students: 
...
For reasons we will see in a later visualization, we are going to exclude outlying areas and freely associated states. 
