...
We chose to use a publicly available dataset for this walkthrough. Like most publicly available datasets, this one contains aggregate data, or data that is not at the level of individuals (individual students).
...
Let's create the raw materials first. Our raw materials will be file paths to each of the CSVs we want to read in. Use `list.files` to make a vector of filenames and their paths and name that vector `filenames`. `list.files` returns a vector of file names that exist in the folder you point to in the `path` argument. When we set the `full.names` argument to "TRUE", we get a full path of the filenames. This will be useful later when we need the file names and their paths to read our data in. 
...
all_files <- filenames %>% map(., ~read_csv(., skip = 4)) # note - this code is not run!
```
...
5. Transform the column formats from wide to narrow using `gather`
3. Change the data types of variables 
3. Explore NAs
...
*This dataset uses a binary approach to data collection about gender. Students are described as either male or female. The need for an inclusive approach to documenting gender identity is discussed in a document by Andrew Park (2016) of The Williams Institute at UCLA.*
