...
If this is your first time working on publicly available education data, congratulations on finding an extremely common problem! By now you've noticed that not only do the number of columns not match, but the column names are not all the same. This is a problem because we won't be able to combine the datasets into one long one. When we try, `bind_rows` returns a dataets with 100 columns instead of the expected 50. 
