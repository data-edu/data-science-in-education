...
The authors chose to use a publicly available dataset for this walkthrough. Like most publicly available datasets, this one contains aggregate data, or data that is not at the level of individuals (in our case, usually individual teachers or students).
...
Here's how we can interpret the estimate column: The estimate of the `Intercept` is 2.03356, which is the estimated value of the `ratio` variable when the `year` variable is 2012. The remaining rows of the model output show how much each year differs from 2012. Fot example, `year2013` has an estimate of -0.01205, which suggests that on average the value of `ratio` is .01205 less than 2.03356. On average, the ratio of `year2014` is .02372 less than 2.03356. The `t value` column tells us the size of difference between the estimated value of the ratio for each year and the estiamted value of the ratio of the intercept. Generally speaking, the larger the t value the larger the chance that any difference is not random. 
