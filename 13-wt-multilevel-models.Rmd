...
```{r}
m_course_bayes <- 
  brm(final_grade ~ TimeSpent + (1|course_id), data = dat)
```

Like with the multilevel model, you can simply type the name of the model to see the output.

```{r}
m_course_bayes
```

Let's briefly contrast this with the multilevel model (`m_course`). You'll see the same parameter for `TimeSpe
