...
If you have not before - as for other packages used for the first time - you'll need to install {lme4}, {performance}, and {brms} once to do the rest of this walkthrough. If helpful, head to the [Packages](#c06p) section of the [Foundational Skills](#c06) chapter for an overview of installing packages.
...
Before we import our data, let's spend some time learning about a process called dummy-coding. In this discussion, we'll see how dummy coding works using the `model.matrix()` function that comes built into R, so no additional packages are needed.

Let's look at the `iris` data that comes built into R. You'll use the `glimpse()` function to do so.
...
How many possible values are there for `Species`? We can check with the `levels` function: 

``` {r iris levels}
levels(iris$Species)
```

The function `model.matrix()` creates a matrix of dummy-coded variables from a formula. Using `~ Species - 1` tells R to create a column for every level of `Species` (the `- 1` removes the intercept so all three levels get their own column). It returns *three* variables, one for each of the three levels of Species - `setosa`, `versicolor`, and `virginica`.
...
m_course_school <- 
  lmer(final_grade ~ TimeSpent_std + (1|course_id) + (1|school_id), data = dat)
```
...
As mentioned earlier, there is a deep connection between multilevel models and Bayesian methods. Helpfully, it is _very_ straightforward to specify Bayesian versions of multilevel models.

Let's use the very last model you specified. You will see a bit more happening ("Compiling Stan program...") as the process is a bit more involved under the hood.

```{r, bayes-model}
m_course_bayes <- 
  brm(final_grade ~ TimeSpent_std + (1|course_id), data = dat)
```

Like with the multilevel model, you can simply type the name of the model to see the output or use `tab_model()`.

```{r show results of m_course_bayes}
tab_model(m_course_bayes,
          title = "Table 13.5")
```
...
Let's briefly contrast this with the multilevel model (`m_course`). You'll see the same parameter for `TimeSpent_std`. But, look at the columns in the output. Instead of a single estimate and a _p_-value, you now have an estimate, an estimate of the error, and a 95% credible interval (`l-95% CI` and `u-95% CI`), which you cam interpret as a confidence interval. That credible interval is the direct probability statement we mentioned earlier: there is a 95% probability that the true effect of `TimeSpent_std` falls between the lower and upper bounds. You can consult @bayesrulesbook for much more on this (credible intervals and how they differ from confidence intervals).

In this case, we can say there is a 95% probability that the effect of time spent is between 7.75 and 11.15. This is a subtly but meaningfully different kind of statement than what a _p_-value tells you. Also, notice how similar the estimates are to those from the multilevel model — the intercept (75.80 vs. 75.63), the effect of `TimeSpent_std` (9.46 vs. 9.45), and the random effect and residual standard deviations are all very close. This is evidnece of the connection between miultilevel and Bayesian models we suggested earlier.
...
There is much more that can be done with multilevel and Bayesian models; we have more
