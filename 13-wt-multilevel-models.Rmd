...
Now that we have some background on dummy codes, let's return to the online science class data. Let's use what we learned about multilevel models and dummy codes to learn about the effect of being in a specific class on a student's grade. 

### Importing the Data 

First we'll load the built-in dataset from the {dataedu} package. 

``` {r, message = F, warning = F}
dat <- dataedu::sci_mo_processed
```

Next, let's look at how many classes there are. These classes will be our factor levels that we'll be using in our model soon. We can use the `count()` function to see how many courses there are: 
